...
LogicalResult OpTrait::impl::verifyTensorLayouts(Operation *op) {
  ...
  auto checkLayout = [&](Value val, auto makeErr) -> LogicalResult {
    ...
    // TODO: What if the encoding is nested within tt.ptr?
  ...
  for (size_t i = 0; i < op->getNumOperands(); i++) {
    ...
    auto err = checkLayout(operand, [&]() {
      ...
      return op->emitError("Operand ")
             << i << " (" << operandStr << ") has an invalid layout: ";
    });
    if (!err.succeeded())
      return err;
  }
  ...
  for (size_t i = 0; i < op->getNumResults(); i++) {
    ...
    auto err = checkLayout(result, [&]() {
      if (op->getNumResults() == 1) {
        // return op->emitError("In op ") << op << ", with result " << result <<
        // " with rank " << result.getType().cast<RankedTensorType>().getRank()
